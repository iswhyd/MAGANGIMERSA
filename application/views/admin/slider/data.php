<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Slider</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Slider</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				...
				<tbody>
					<?php $no=1; foreach($slider->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->ket_slider; ?></td>
						<td><img src="<?php echo base_url($r->foto_slider); ?>" alt="" width="120px"></td>
						<td><b><?php echo $r->status_slider; ?></b></td>
						<td>
							<?php if($r->status_slider=='on'){
								echo '<a href="'.base_url('slider/offkan/'.$r->id_slider).'" class="btn btn-warning btn-xs">Off</a>';
							}else{
								echo '<a href="'.base_url('slider/onkan/'.$r->id_slider).'" class="btn btn-success btn-xs">On</a>';
							} ?>
							<a href="#" onclick="hapus('<?php echo $r->id_slider; ?>')" class="btn btn-danger btn-xs"><i class="fa fa-remove"></i> Hapus</a>
						</td>
					</tr>
					<?php $no++; } ?>
				</tbody>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahslider'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editslider/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
