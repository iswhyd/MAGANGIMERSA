<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Statistik Agama</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Statistik Agama</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				...
				<tbody>
					<?php $no=1; foreach($agamapenduduk->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->tahun; ?></td>
						<td><?php echo $r->kdagama; ?></td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahagamapenduduk'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editagamapenduduk/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		swal({
		  title: "Menghapus agamapenduduk Ini?",
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('agamapenduduk/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("agamapenduduk Berhasil Dihapus", {
				      icon: "success",
				   });
