<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data APBD Pelaksanaan</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah APBD Pelaksanaan</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				<thead>
					...
					<th>Pembelanjaan</th>
					<th>Pagu Pembelanjaan</th>
					<th>Pembiayaan</th>
					<th>Pagu Pembiayaan</th>
					<th width="15%">Aksi</th>
				</thead>
				<tbody>
					<?php $no=1; foreach($apbdespel->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->tahun; ?></td>
						<td>Rp. <?php echo rp($r->dapat); ?></td>
						<td>Rp. <?php echo rp($r->pagudapat); ?></td>
						<td>Rp. <?php echo rp($r->belanja); ?></td>
						<td>Rp. <?php echo rp($r->pagubelanja); ?></td>
						<td>Rp. <?php echo rp($r->biaya); ?></td>
						<td>Rp. <?php echo rp($r->pagubiaya); ?></td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahapbdespel'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editapbdespel/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		swal({
		  title: "Menghapus APBD Pelaksanaan Ini?",
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('apbdespel/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("apbdespel Berhasil Dihapus", {
				      icon: "success",
				   });
