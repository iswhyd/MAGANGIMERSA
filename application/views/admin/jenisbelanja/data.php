<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Jenis Belanja</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Jenis Belanja</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				...
				<tbody>
					<?php $no=1; foreach($jenisbelanja->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->kd_jenisbelanja; ?></td>
						<td><?php echo $r->nama_jenisbelanja; ?></td>
						<td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahjenisbelanja'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editjenisbelanja/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		swal({
		  title: "Menghapus Jenis Belanja Ini?",
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('jenisbelanja/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("jenisbelanja Berhasil Dihapus", {
				      icon: "success",
				   });
