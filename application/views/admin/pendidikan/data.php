<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Pendidikan</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Pendidikan</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				...
				<tbody>
					<?php $no=1; foreach($pendidikan->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->kd_pendidikan; ?></td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahpendidikan'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editpendidikan/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		swal({
		  title: "Menghapus Pendidikan Ini?",
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('pendidikan/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("pendidikan Berhasil Dihapus", {
				      icon: "success",
				   });
