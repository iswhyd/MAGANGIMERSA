<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Penduduk</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Penduduk</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				...
				<tbody>
					<?php $no=1; foreach($penduduk->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->tahun; ?></td>
						<td><?php echo $r->cowok; ?></td>
						<td><?php echo $r->cewek; ?></td>
						<td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahpenduduk'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editpenduduk/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('penduduk/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("penduduk Berhasil Dihapus", {
				      icon: "success",
				   });
