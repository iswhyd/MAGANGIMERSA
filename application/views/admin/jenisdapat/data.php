<div class="">
	<div class="box box-primary">
		<div class="box-header with-border">
			<h3 class="box-title"><b>Data Jenis Pendapatan</b></h3>
			<span class="pull-right"><a href="#" onclick="tambah()" class="btn btn-success btn-xs">Tambah Jenis Pendapatan</a></span>
		</div>
		<div class="box-body">
			<table class="table table-bordered table-hover example1">
				<thead>
					<th width="5%">No</th>
					<th>Kode Jenis Pendapatan</th>
					<th>Nama Jenis Pendapatan</th>
					<th width="20%">Aksi</th>
				</thead>
				<tbody>
					<?php $no=1; foreach($jenisdapat->result() as $r){ ?>
					<tr>
						<td><?php echo $no; ?></td>
						<td><?php echo $r->kd_jenisdapat; ?></td>
						<td><?php echo $r->nama_jenisdapat; ?></td>
						<td>
...
<script>
	function	tambah(){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/tambahjenisdapat'); ?>",
			cache : false,
			async : false,
	...
	function	edit(id){
		$.ajax({
			type : "GET",
			url : "<?php echo base_url('admin/editjenisdapat/'); ?>"+id,
			cache : false,
			async : false,
			success : function(html){
				$("#edit").html(html).show();
	...
	function hapus(id){
		swal({
		  title: "Menghapus Jenis Pendapatan Ini?",
		...
		.then(function () {
        	$.ajax({
				type : "GET",
				url : "<?php echo base_url('jenisdapat/hapus/'); ?>"+id,
				cache : false,
				async : false,
				success : function(response){
					swal("jenisdapat Berhasil Dihapus", {
				      icon: "success",
				   });
